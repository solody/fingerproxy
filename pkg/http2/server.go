...
import (
	...
	"github.com/wi1dcard/fingerproxy/pkg/http2/internal/httpcommon"
...
func (sc *serverConn) processFrame(f Frame) error {
	...
	switch f := f.(type) {
	case *SettingsFrame:
		// fingerproxy: capture SETTINGS for HTTP/2 fingerprinting
		if !f.IsAck() {
	...
	case *MetaHeadersFrame:
		// fingerproxy: capture HEADERS (+ optional priority) for fingerprinting
		if md, ok := metadata.FromContext(sc.baseCtx); ok {
			...
			// Build header order string (header names only)
	...
	case *WindowUpdateFrame:
		// fingerproxy: capture first WINDOW_UPDATE increment for fingerprinting
		if md, ok := metadata.FromContext(sc.baseCtx); ok {
	...
	case *PriorityFrame:
		// fingerproxy: capture PRIORITY frames for fingerprinting
		if md, ok := metadata.FromContext(sc.baseCtx); ok {
